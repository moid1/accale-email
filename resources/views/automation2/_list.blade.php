@if ($automations->count() > 0)
    <table id="Automation2ListTable" class="table table-box pml-table mt-2"
                            ...
                            <ul class="dropdown-menu dropdown-menu-end">
                                ...
                                @can('disable', $automation)
                                    <li>
                                        <a
                                            class="dropdown-item list-action-single"
                                            link-method="POST" link-confirm="{{ trans('messages.disable_automations_confirm') }}"
                                            href="{{ action('Automation2Controller@disable', ["uids" => $automation->uid]) }}"
                                        >
                                            <span class="material-symbols-rounded me-2">hide_source</span> {{ trans('messages.disable') }}
                                        </a>
                                    </li>
                                @endcan
                                @can('create', $automation)
                                    <li>
                                        <a
                                            list-action="copy"
                                            class="dropdown-item"
                                            href="{{ action('Automation2Controller@copy', $automation->uid) }}"
                                        >
                                            <span class="material-symbols-rounded me-2">content_copy</span>
                                            {{ trans('messages.automation.copy') }}
                                        </a>
                                    </li>
                                @endcan
                                @if (\Gate::allows('delete', $automation))
                                    <li>
                                        ...
                                        >
                                            <span class="material-symbols-rounded me-2">delete_outline</span> {{ trans("messages.delete") }}
                                        </a>
                                    </li>
                                @endif
                            </ul>
    ...
    <script>
        var Automation2List = {
            showCopyPopup: function(copyUrl) {
        ...
        $(function() {
            $('#Automation2ListTable [list-action="copy"]').on('click', function(e) {
                e.preventDefault();

                var copyUrl = $(this).attr('href');

                Automation2List.showCopyPopup(copyUrl);
            });
        });
    </script>
