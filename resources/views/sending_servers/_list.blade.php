        ...
        @foreach ($items as $key => $item)
            <tr>
                ...
                <td>
                    ...
                    @else
                        <h5 class="m-0 text-bold">
                            <a class="kq_search d-block" href="{{ action('SendingServerController@edit', ["id" => $item->uid, "type" => $item->type]) }}">{{ $item->name }}</a>
                        </h5>
                    @endif
                ...
                <td>
                    <div class="single-stat-box pull-left ml-4">
                        ...
                        @else
                            <span class="no-margin stat-num kq_search">{{ trans('messages.sending_server.type.' . $item->type) }}</span>
                        @endif
                        <br />
                ...
                <td>
                    <div class="single-stat-box pull-left ml-4">
                        <span class="text-muted"><strong>{{ number_with_delimiter($item->getCreditsUsed('send')) }}</strong> {{ trans('messages.sending_server.email_sent') }}</span>
                        <br />
                        ...
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu dropdown-menu-end">
                                @if (Auth::user()->customer->can('enable', $item))
                                    <li>
                                        <a class="dropdown-item list-action-single" link-confirm="{{ trans('messages.enable_sending_servers_confirm') }}" href="{{ action('SendingServerController@enable', ["uids" => $item->uid]) }}">
