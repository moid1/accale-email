								...
								<div class="segment-conditions-container">
									@foreach ($segment->segmentConditions as $condition)
										<div class="row condition-line" rel="{{ $condition->uid }}">
											...
											<div class="col-md-7 operator_value_col" data-url="{{ action('SegmentController@conditionValueControl') }}">
												@php
                                                    $cField = (
                                                                $condition->field_id ?
                                                                    (
                                                                        Acelle\Model\Field::findByUid($condition->field_id) ??
                                                                            (Acelle\Model\Field::find($condition->field_id) ?? NULL)
                                                                    )
                                                                    : NULL
                                                    );
                                                @endphp
											...
											<div class="col-md-1">
												<a onclick="$(this).parents('.condition-line').remove()" href="#delete" class="btn bg-danger-400"><span class="material-symbols-rounded">delete_outline</span></a>
											</div>
										</div>
									@endforeach
								</div>
