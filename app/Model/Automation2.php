...
class Automation2 extends Model
{
    ...
    public function getSubscribersWithDateOfBirth()
    {
        ...
        $query = $this->subscribers()
            ->leftJoin('subscriber_fields', function ($join) use ($dobFieldId) {
                $join->on('subscribers.id', '=', 'subscriber_fields.subscriber_id');
                $join->on('subscriber_fields.field_id', DB::raw($dobFieldId));
            })
            /*
    ...
    public function checkForSpecificDatetime()
    {
        ...
        $eventDate = $this->customer->parseDateTimeFromUtcString(sprintf('%s %s', $trigger->getOption('date'), $trigger->getOption('at')));
